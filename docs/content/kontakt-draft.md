# Kontakt Draft

## Ziel

Die Seite soll die Kontaktaufnahme so einfach wie moeglich machen und alle Basisdaten direkt sichtbar halten.

## Primärer Intent

`Kontakt Hundetraining Hamburg`

## Kernbotschaft

Wenn du Fragen hast oder ein Erstgespraech anfragen moechtest, erreichst du Beziehungssache Hund direkt per E-Mail, Telefon oder Anfrageformular.

## Muss enthalten

- Name
- Adresse
- E-Mail
- Telefon
- kurze Kontakt-Einladung
- CTA

## Empfohlene Struktur

### Intro

- Headline: Kontakt zu Beziehungssache Hund
- Subline: Schreibe mir oder ruf an, wenn du ein Erstgespräch oder ein passendes Training anfragen moechtest
- Kurzüberblick: Telefon, E-Mail, Adresse, Anfrage zu Erstgespräch, Einzeltraining oder ergänzenden Angeboten

### Kontaktblock

- Beziehungssache Hund
- Jacky Rebien
- Bundesstr. 74, 20144 Hamburg
- info@beziehungssache-hund.de
- 015228385291

### Was du anfragen kannst

- Erstgespraech
- Einzeltraining
- DOGSpace
- Coaching mit Hund

### Hinweise

- Du-Ansprache
- keine englischen Template-Woerter
- keine generische Office-Hours aus einem Baukasten

### CTA

- Nachricht senden
- Anrufen

## Was die Seite nicht tun soll

- nicht wie eine generische Kontaktseite wirken
- keine Platzhaltertexte enthalten
- keine widerspruechlichen Oeffnungszeiten zeigen
- keine heroartige Bildfläche aufbauen
